...
% Problem II: Running the face detector for different angles [0, 30, -30]
...
% Function to check if there is a face in the image. This time it checks for different face angles
function face_exist = estimate_face(I_FG)
    face = vision.CascadeObjectDetector('MergeThreshold', 7);
    angles = [0, 30, -30];
    BB = [];
    
    for angle = angles  % For each angle, check for a fact and if there is a face, break the for loop
        rotated_I_FG = imrotate(I_FG, angle);
        BB = face(rotated_I_FG);
        if ~isempty(BB)
            break;
        end
    end
    ...
    imshow(I_FG);
    if ~isempty(BB)   % If there is a face, draw the rectangle
        for i = 1:size(BB, 1)
